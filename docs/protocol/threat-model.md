...
1. No permanent public controller, wallet, account, installation, or device identifier.
2. Each disposable identity uses independently generated random private key material.
3. RP-to-identity scope mappings and local labels remain local or inside opaque encrypted backup
   ciphertext.
...
7. Logs exclude private keys, revocation/backup secrets, raw proofs, routine subjects, IP addresses,
   and user agents.
...
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D1/R2/operational database compromise | No civil-identity map or server-custodied user keys; backups are client ciphertext; registry state is public cryptographic data only.                        |
...
| Correlating service operator          | No RP audience at registration, no proof transit through Nexus, opaque backups, minimal short-lived telemetry, and aggregate-only metrics.                   |
