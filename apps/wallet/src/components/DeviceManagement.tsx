      ...
      {issued === undefined ? (
        <form onSubmit={(event) => void submit(event)} className="space-y-5">
          ...
          <div className="rounded-xl border border-indigo-100 bg-indigo-50 p-4 text-sm leading-relaxed text-indigo-950">
            ...
            only inside an encrypted transfer file.
          </div>
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-relaxed text-amber-950">
            ...
            Copies of the file and key can create indistinguishable clones of this device. Keep them
            separate, install once, then remove both transfer copies.
          </div>
        ...
        <div className="space-y-5">
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm leading-relaxed text-emerald-950">
            ...
            Download the encrypted file, then copy the key separately. The key is not in the file.
          </div>
          <button
            type="button"
            onClick={() =>
...
export function InstallDeviceModal({ onClose, onInstall }: InstallDeviceModalProps) {
  ...
  return (
    ...
    >
      <form onSubmit={(event) => void submit(event)} className="space-y-5">
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-relaxed text-amber-950">
          The file and its 43-character key must have arrived separately. A copied bundle can create
          a clone, so remove transfer copies after a successful installation.
        </div>
        <div>
          <label
